...
def test_default_dec(caplog):
    decorated_test_class = apply_decorator(SpuriousClass)
    ...
    assert "new: SpuriousClass" in msg
    assert ARG1 in msg
...
def test_with_logger_object(caplog, test_logger):
    decorated_test_class = apply_decorator(SpuriousClass, logger=test_logger)
    with caplog.at_level(logging.DEBUG, logger=test_logger.name):
        instance = decorated_test_class(ARG1, kwarg2=KWARG2)  # noqa: F841
    assert "new: SpuriousClass" in caplog.records[0].msg


def test_no_logargs(caplog, test_logger):
    caplog.clear()
    decorated_test_class = apply_decorator(SpuriousClass, logger=test_logger.name, logargs=False)
    instance = decorated_test_class(ARG1, kwarg2=KWARG2)  # noqa: F841
    msg = caplog.records[0].msg
    assert "new: SpuriousClass" in msg
...
def test_log_defaults(caplog, test_logger):
    ...
    decorated_test_class = apply_decorator(SpuriousClass, logger=test_logger, logargs=True, logdefaults=True)
    with caplog.at_level(logging.DEBUG, logger=test_logger.name):
        instance = decorated_test_class(ARG1)  # noqa: F841

    assert "new: SpuriousClass" in caplog.text
...
def test_with_callable(caplog, test_logger):
    ...
    decorated_test_class = apply_decorator(SpuriousClass, logger=logger_callable)
    with caplog.at_level(logging.DEBUG, logger=test_logger.name):
        instance = decorated_test_class(ARG1)  # noqa: F841
...
def test_invalid_logger(caplog, test_logger):
    with pytest.raises(TypeError):
        decorated_test_class = apply_decorator(SpuriousClass, logger=123)
        with caplog.at_level(logging.DEBUG, logger=test_logger.name):
            instance = decorated_test_class(ARG1)  # noqa: F841
